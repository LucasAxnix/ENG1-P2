...
public class AI extends Boat {
    ...
    private Vector2 getAIPredictionVector(float distance) {
        ...
        // First we need to calculate the position of the boat's head (the front of the
        // boat)
    ...
    private boolean obstaclesInRange() {
        for (Obstacle obstacle : this.lane.obstacles) {
            ...
            // Check for obstacles
            if (boatRightX >= posX && boatLeftX <= posX + width && objectChecker.y >= posY
                    && boatSprite.getY() + boatSprite.getHeight() / 2 <= posY) {
